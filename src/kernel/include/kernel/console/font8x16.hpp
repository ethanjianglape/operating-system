...
namespace kernel::console::fonts {
    ...
    inline constexpr std::uint8_t font8x16[NUM_GLPHYS][FONT_HEIGHT] = {
        // 32: ' ' (space)
    ...
    // Get font bitmap for a character (returns nullptr for unsupported chars)
    inline const std::uint8_t* get_glyph(char c) {
        if (c < 32 || c > 126) {
