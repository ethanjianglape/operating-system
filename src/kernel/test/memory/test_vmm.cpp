...
void test_raw_page_returns_non_null()
{
    ...
    arch::vmm::free_kernel_page(page);
}
...
{
    auto* page = static_cast<std::uint8_t*>(arch::vmm::alloc_kernel_page());
...
{
    void* page1 = arch::vmm::alloc_kernel_page();
    arch::vmm::free_kernel_page(page1);

    void* page2 = arch::vmm::alloc_kernel_page();
    test::assert_not_null(page2, "raw page allocation after free succeeds");
    arch::vmm::free_kernel_page(page2);
}

// Tracked allocation tests
void test_contiguous_memory_returns_non_null()
{
    void* mem = arch::vmm::alloc_kernel(128);
    test::assert_not_null(mem, "alloc_contiguous_memory returns non-null");
    arch::vmm::free_kernel(mem);
}

void test_contiguous_memory_is_writable()
{
    constexpr std::size_t SIZE = 1024;
    auto* mem = static_cast<std::uint8_t*>(arch::vmm::alloc_kernel(SIZE));
...
{
    void* mem1 = arch::vmm::alloc_kernel(512);
    arch::vmm::free_kernel(mem1);

    void* mem2 = arch::vmm::alloc_kernel(512);
    test::assert_not_null(mem2, "contiguous memory allocation after free succeeds");
    arch::vmm::free_kernel(mem2);
}

void test_sequential_allocs_differ()
{
    void* mem1 = arch::vmm::alloc_kernel(64);
    void* mem2 = arch::vmm::alloc_kernel(64);
